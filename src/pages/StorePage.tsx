...
const StorePage = () => {
  ...
  const fetchStoreOrders = async (storeId: string) => {
    ...
    try {
      // Fetch all orders that contain products from this store
      ...
      const typedOrders: StoreOrder[] = storeOrders.map(order => ({
        ...order,
  ...
  const handleProductSave = async (product: Product) => {
    try {
      ...
      // Clean up and validate product data
      ...
      const productData = {
        ...
        image: product.image?.trim() || 'https://via.placeholder.com/150', // Default image
        category: product.category?.trim() || 'Общая',
      ...
      if (!data || data.length === 0) {
        throw new Error('No data returned from product save operation');
      }
  ...
  useEffect(() => {
    ...
    // Subscribe to order changes for this store
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Check if this order contains products from our store
          const order = payload.new || payload.old;
          if (order?.items && Array.isArray(order.items)) {
            const hasStoreProduct = order.items.some((item: any) => 
              item.product && item.product.store_id === selectedStore.id
            );
            
            if (hasStoreProduct) {
              // Refresh orders for this store
              fetchStoreOrders(selectedStore.id);
